...
extension JSValue : CustomStringConvertible {
    ...
    public func stringify(indent: Int) -> String {
        let range = 0..<indent
        let padding = range.reduce("", { s, i in return s + " " })
...
extension JSValue {
    func prettyPrint(_ indent: String, _ level: Int) -> String {
        let levels = 0...level
        let currentIndent = indent == "" ? "" : levels.map({ (item: Int) in "" }).joined(separator: indent)
        ...
        case .JSObject(let dict):
            return "{\(newline)" + dict.map({ "\(nextIndent)\"\($0)\":\(space)\($1.prettyPrint(indent, level + 1))"}).joined(separator: ",\(newline)") + "\(newline)\(currentIndent)}"
